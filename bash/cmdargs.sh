...
case $1 in
      -v | --verbose )
          shift
          echo "Found Verbose Mode Command on Command Line"
    exit
          ;;
      -h | --help )
          shift
    echo "Found Help Command on Command Line"
          exit
          ;;
-d | --debug )
  if [ ! -z $2 ] && [ $2 -ge 0 ]
  then
                  level=$2
      echo "Found Debug Command on Command Line of level=$level"
      shift
      exit
  else
      shift
      echo "Debug requires level"
      exit
  fi
  ;;
      * )
    shift
    useafter=$1
          echo "$useafter not a recognized option"
    exit
  esac
